--------------------------- MODULE getAllSuffixes ---------------------------
EXTENDS Integers, Sequences, TLC
CONSTANT input

(*
    --algorithm getAllSuffixes {
        variable result = input, i = 1, j = 1, dummy = << >>;
        
        
       \* main method
       {
            assert Len(input) > 0;
       
            while(i <= Len(input)) {
                dummy := << >>;
                
                j := i;
                while(j <= Len(input)) {
                    dummy := dummy \o <<input[j]>>;
                    
                    j := j + 1;
                };
                
                result[i] := dummy;
                
                i := i + 1;
            };
            
            
           assert (\A k \in 1..Len(input): 
           (\A l \in 1..Len(input): result[k][l] = input[k - 1 + l])
           );
       } 
    }
*)
\* BEGIN TRANSLATION (chksum(pcal) = "9f8befe1" /\ chksum(tla) = "8a044da8")
VARIABLES result, i, j, dummy, pc

vars == << result, i, j, dummy, pc >>

Init == (* Global variables *)
        /\ result = input
        /\ i = 1
        /\ j = 1
        /\ dummy = << >>
        /\ pc = "Lbl_1"

Lbl_1 == /\ pc = "Lbl_1"
         /\ Assert(Len(input) > 0, 
                   "Failure of assertion at line 12, column 13.")
         /\ pc' = "Lbl_2"
         /\ UNCHANGED << result, i, j, dummy >>

Lbl_2 == /\ pc = "Lbl_2"
         /\ IF i <= Len(input)
               THEN /\ dummy' = << >>
                    /\ j' = i
                    /\ pc' = "Lbl_3"
               ELSE /\ Assert(       (\A k \in 1..Len(input):
                              (\A l \in 1..Len(input): result[k][l] = input[k - 1 + l])
                              ), 
                              "Failure of assertion at line 30, column 12.")
                    /\ pc' = "Done"
                    /\ UNCHANGED << j, dummy >>
         /\ UNCHANGED << result, i >>

Lbl_3 == /\ pc = "Lbl_3"
         /\ IF j <= Len(input)
               THEN /\ dummy' = dummy \o <<input[j]>>
                    /\ j' = j + 1
                    /\ pc' = "Lbl_3"
                    /\ UNCHANGED << result, i >>
               ELSE /\ result' = [result EXCEPT ![i] = dummy]
                    /\ i' = i + 1
                    /\ pc' = "Lbl_2"
                    /\ UNCHANGED << j, dummy >>

(* Allow infinite stuttering to prevent deadlock on termination. *)
Terminating == pc = "Done" /\ UNCHANGED vars

Next == Lbl_1 \/ Lbl_2 \/ Lbl_3
           \/ Terminating

Spec == Init /\ [][Next]_vars

Termination == <>(pc = "Done")

\* END TRANSLATION 

=============================================================================
\* Modification History
\* Last modified Sun Mar 05 20:35:28 EST 2023 by chiddy00
\* Created Sun Mar 05 00:32:17 EST 2023 by chiddy00
