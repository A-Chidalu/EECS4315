    ...
    --algorithm getLeftShifts {
        ...
        \* main method
        {
           ...
           beginWhile: while(i <= Len(input)) {
                ...
                calNewPos: newPos := (((i - 1) - normalizeN + n) % Len(input)) + 1;
            ...
            doAssertion: assert \A x \in 1..Len(input): input[x] = result[(((x - 1) - normalizeN + n) % Len(input)) + 1];
        }
    }
*)
\* BEGIN TRANSLATION (chksum(pcal) = "7c67bb7b" /\ chksum(tla) = "f13c9175")
...
calNewPos == /\ pc = "calNewPos"
             /\ newPos' = (((i - 1) - normalizeN + n) % Len(input)) + 1
             /\ pc' = "assignResult"
...
doAssertion == /\ pc = "doAssertion"
               /\ Assert(\A x \in 1..Len(input): input[x] = result[(((x - 1) - normalizeN + n) % Len(input)) + 1], 
...
\* Last modified Mon Mar 06 14:49:44 EST 2023 by chiddy00
